...
2. Si **on les spécifie** et on ne rajoute pas strict_mode=1, **PHP essayera de faire le casting quand il peut**. S'il ne peut pas (ex: transformer un string qui ne represente pas un numéro en int) on aura une erreur.

3. Si **on les spécifie et on rajoute strict_mode=1**, on doit **respecter completement** le compromis et les types des paramètres doit être respecté. 
